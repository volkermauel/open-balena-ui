...
Every created release row SHALL carry `start_timestamp` alongside `status: 'success'`, so the instance database's NOT
NULL constraint on `release.start_timestamp` is satisfied.

#### Scenario: Seed a version

- **WHEN** a client seeds a hostOS version
- **THEN** the created release row includes `start_timestamp` and the instance API accepts the creation
